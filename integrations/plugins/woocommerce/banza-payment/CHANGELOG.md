...
- Webhook endpoint registered at `https://your-store.com/wc-api/banza`
...
- `transaction.refunded` webhook handler: marks order as "Refunded"; guarded by `_banza_refund_processed` meta to prevent duplicate processing
- Order meta storage: `_banza_transaction_id` (Banza transaction UUID), `_banza_amount_minor` (integer minor units sent to API), `_banza_refund_processed` (refund idempotency flag)
