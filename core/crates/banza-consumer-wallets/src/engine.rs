...
use banza_ledger::{Account, AccountType, LedgerEngine};
use banza_types::{ConsumerId, ConsumerWalletId, Currency, LedgerEntryId, LedgerPostingId, Money};
    ...
    WR: ConsumerWalletRepository,
{
    ...
    async fn provision_ledger_accounts(
        ...
        label_prefix: &str,
        currency:     Currency,
    ) -> Result<(banza_types::AccountId, banza_types::AccountId), ConsumerWalletError> {
        let available = self
            .ledger
    ...
    WR: ConsumerWalletRepository,
{
    ...
    async fn resolve_to_wallet(
        ...
        let normalized = banza_identity::normalize_handle(handle);

        // Syntax gate — rejects malformed handles before any DB round-trip.
        banza_identity::validate_handle(&normalized)
...
// ---------------------------------------------------------------------------
// Handle validation — delegates to banza-identity as single source of truth
// ---------------------------------------------------------------------------

fn validate_handle_format(handle: &str) -> Result<(), ConsumerWalletError> {
    banza_identity::validate_handle(handle)
        .map_err(ConsumerWalletError::InvalidHandle)
}
...
mod tests {
    ...
    use banza_ledger::{Account, LedgerEngine, LedgerEntry, LedgerPosting};
    use banza_types::{AccountId, ConsumerId, ConsumerWalletId, Currency, Money};
    use chrono::Utc;
    use uuid::Uuid;
    ...
    impl LedgerEngine for MockLedger {
        async fn create_account(
            &self,
            account: Account,
        ) -> Result<Account, banza_ledger::LedgerError> {
        ...
        ) -> Result<Money, banza_ledger::LedgerError> {
        ...
        async fn reverse(
            ...
            use banza_ledger::{EntryType, PostingBuilder};
            ...
            let reversal = builder.build().map_err(|_| banza_ledger::LedgerError::InsufficientEntries)?;
            self.post(reversal).await
        }

        async fn get_posting(
            &self,
            _posting_id: banza_types::LedgerPostingId,
        ) -> Result<LedgerPosting, banza_ledger::LedgerError> {
