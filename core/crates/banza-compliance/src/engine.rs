...
use banza_types::{CustomerId, MerchantId, Money};
...
impl<R: ComplianceRepository> ComplianceEngine for PostgresComplianceEngine<R> {
    ...
    async fn check_merchant_can_transact(
        ...
        // Amount limits on merchants are enforced by the risk engine (banza-risk).
...
mod tests {
    ...
    use banza_types::{Currency, CustomerId, MerchantId, Money};
