...
**Supersedes:** ADR-016 (Arquitectura de Marca Banzami/Banza)

---

## Context

The legally and commercially approved company name, registered with Guiché Único da Empresa, is **Banzami**.
...
This creates a critical mismatch: the legally approved name ("Banzami") is used for the invisible infrastructure layer, while the user-facing product ("Banza") carries a name that is not the registered entity. Commercial communications, contracts, and investor materials must reference the legal name — which is Banzami — yet the product the user actually pays with is called Banza.
...
|------|------------------------|---------------------|
| Protocol / ecosystem / open infrastructure | Banzami | **Banza** |
| Main product / reference operator / commercial payment experience | Banza | **Banzami** |
...
### Banzami — Main Product and Reference Operator

**Banzami** becomes the name of the first and main product built on Banza — the reference operator and the commercial payment experience used by consumers and merchants.
...
**BanzAI** replaces BanzamIA as the Protocol Operating System for the Banza ecosystem.
...
Every occurrence of Banzami, Banza, and BanzamIA across documentation, code, APIs, and public copy must be **classified before renaming**. The semantic meaning of each occurrence determines which name replaces it.
...
|---------|-------------------|
| Protocol references | "Banza Protocol — formerly Banzami Protocol" |
| Product references | "Banzami — formerly Banza" |
| AI OS references | "BanzAI — formerly BanzamIA" |
...
**Banzami as product:** The legal name must be the user-facing name. When a consumer makes a payment, the name they see, the name on their receipt, and the name in their bank statement should all be the same name the company is registered under. Using Banza for the product and Banzami for the invisible protocol inverts this natural relationship.

**Banza as protocol:** Shorter, cleaner, better suited for a protocol/infrastructure layer. "Built on Banza" is a stronger developer positioning statement than "Built on Banzami" when Banzami is also the name of a competing product built on the same protocol.

**BanzAI over BanzamIA:** The suffix "IA" (Inteligência Artificial) is Portuguese-specific and opaque to international audiences. "AI" is universally understood. Removing the etymological link to "Banzami" also prevents the AI OS from being associated with the product rather than the protocol.
...
- Cleaner developer narrative: "Banza Protocol" as infrastructure, "Banzami" as product built on it
...
- **ADR-016** (superseded): Arquitectura de Marca Banzami/Banza — established the now-inverted naming
...
- **ADR-024**: Reference Operator — Banza/Banzami relationship (content unchanged, naming will update)
