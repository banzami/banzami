...
Security architecture for the Banza payment integration: secret management, webhook verification, replay protection, environment isolation, and logging rules.

---

## Threat Model

The Doa × Banza integration faces three primary threat surfaces:
...
Banza integration uses three secrets:
...
|-----------|-------------|
| `t` | Unix timestamp (seconds) when Banza computed the signature |
...
**Why not `req.json()` first?** JSON parsing normalizes the byte sequence — it may remove insignificant whitespace, reorder keys (in some parsers), or change number representations. The verified signature was computed over Banza's original byte sequence. Re-serializing a parsed object produces a different byte sequence, invalidating the HMAC.
...
- Legitimate retries from Banza (up to 2 hours after the first attempt) use fresh timestamps and recompute the signature — they are not replays.
- Clock skew between Banza and Doa's server is tolerated within the window. 5 minutes is standard for webhook implementations (used by Stripe, PayPal, and others).
...
A `bz_test_` API key cannot be used against the live gateway, and vice versa. This is enforced by Banza's infrastructure — not a convention.
...
The webhook endpoint must be served over HTTPS. Banza rejects endpoint registration for HTTP URLs (non-`https://` URLs return a validation error at registration time).
...
For local development, use ngrok or cloudflared — both provide HTTPS tunnels. Do not expose a plain HTTP webhook endpoint to Banza even in sandbox.
...
**Why no API key on the endpoint**: The webhook receiver is public — Banza sends to it without any prior authentication. The HMAC signature serves as the authentication mechanism. This is the standard webhook security pattern used across the industry.
...
No card numbers, CVVs, or magnetic stripe data are processed or stored. The Banza consumer app handles cardholder interaction and transmits card data directly to Banza's PCI-compliant infrastructure — Doa is not in the card data flow.
...
Banza-initiated requests do not originate from a browser — CSRF protections are irrelevant. The HMAC signature is the complete authentication mechanism.
