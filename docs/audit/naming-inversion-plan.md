...
The legally approved company name is **Banzami**. The current architecture (ADR-016) assigns "Banzami" to the protocol/infrastructure layer and "Banza" to the user-facing product — the opposite of what the legal registration requires.

ADR-025 formally inverts the naming:

| Role | Before | After |
|------|--------|-------|
| Protocol / ecosystem / kernel | Banzami | **Banza** |
...
- [ ] Wave 3: AI OS rename (BanzamIA → BanzAI, components, routes, identifiers)
...
The SDK name is unresolved. "Banza SDK" (built on the Banza protocol) is the natural fit after inversion, but the packages are currently published as `banza-*`. A separate decision is required before renaming any SDK packages.
