...
def run_manifest(base_url: str) -> list:
    ...
    # MAN-001
    c = make_case("MAN-001", "Operator manifest responds at /.well-known/banzami/operator.json")
    t0 = time.monotonic()
    try:
        status, body = http_request(f"{base_url}/.well-known/banzami/operator.json")
        ms = int((time.monotonic() - t0) * 1000)
        ...
        for field in ["operator_id", "environment", "simulated", "production_allowed", "capabilities"]:
            ok, a = check_field(body, field)
            assertions.append(a)
        c["request"] = {"method": "GET", "path": "/.well-known/banzami/operator.json"}
        c["response"] = {"status": status, "body": body}
        if all(a["passed"] for a in assertions):
            pass_case(c, ms, assertions)
        else:
...
def main():
    ...
    operator_id = None
    try:
        _, manifest = http_request(f"{base_url}/.well-known/banzami/operator.json")
