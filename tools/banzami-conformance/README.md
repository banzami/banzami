...
python3 tools/banzami-conformance/run.py --url http://localhost:3000

# Run up to Level 2 only
python3 tools/banzami-conformance/run.py --url http://localhost:3000 --level 2

# Run a single suite
python3 tools/banzami-conformance/run.py --url http://localhost:3000 --suite health

# Write report to file (quiet mode)
python3 tools/banzami-conformance/run.py --url http://localhost:3000 --output report.json --quiet
```

Or via the shell wrapper:

```bash
./tools/banzami-conformance/run.sh --url http://localhost:3000 --output report.json
```
