...
const MOCK_RESPONSES: Record<TaskType, string> = {
  docs: 'Based on the Banzami protocol documentation, I can answer your question. [mock response — configure BANZAMIA_MODE=live-ai and a vLLM endpoint for real model inference]',
  code: '```typescript\n// Mock code response — configure BANZAMIA_VLLM_URL for Qwen Coder inference\nconsole.log("BanzamIA code generation requires a configured vLLM endpoint");\n```',
  reasoning: 'This requires deep reasoning about the protocol. [mock response — configure DeepSeek via BANZAMIA_VLLM_URL for reasoning tasks]',
